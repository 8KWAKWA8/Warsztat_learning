...
const DraggableComponent = (props: DraggableComponentProps) => {
  ...
  return (
    <div
      ...
      tabIndex={0}
    >
      <span class={[styles.draggableEmote, styles.draggableEmoteLeft].join(' ')}>{props.emote}</span>
      <span class={styles.draggableName}>{props.name}</span>
      <span class={[styles.draggableEmote, styles.draggableEmoteRight].join(' ')}>{props.emote}</span>
    </div>
  );
};
