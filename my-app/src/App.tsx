...
const App: Component = () => {
	...
	const [loadingPair, setLoadingPair] = createSignal<[string, string] | null>(
		null,
	);
	...
	onMount(async () => {
		...
		const alreadyDiscovered = backendItems
	...
	const generateId = (itemName: string) => {
		const existingIds = components().filter(
	...
	const updatePosition = (
		...
		setComponents((prev) =>
			prev.map((comp) => (comp.id === id ? { ...comp, pos: newPos } : comp)),
		);
	};
	...
	const handleRelease = async (id: string) => {
		...
		const overlappingComps = components().filter(
			(c) => c.id !== id && c.visible && isOverlapping(id, c.id),
		);
		...
		<div class={styles.app}>
			<div class={styles.sidebar}>
				...
				<div class={styles.sidebarButtonsGrid}>
					<For
						each={discovered().filter((itemName) =>
							...
							{(comp) => {
								...
								if (currentTouched) {
									if (comp.id === currentTouched) {
										...
										comp.visible &&
										isOverlapping(currentTouched, comp.id)
									) {
										highlight = true;
									}
								}
